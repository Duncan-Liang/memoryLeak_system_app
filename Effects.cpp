...
#ifdef WIND_DEF_AUDIOWIZARD
void AudioFlinger::EffectModule::process(void *buffer, size_t count)
{
    Mutex::Autolock _l(mLock);

    mConfig.outputCfg.accessMode = EFFECT_BUFFER_ACCESS_WRITE;

    mConfig.inputCfg.buffer.frameCount = count;
    mConfig.outputCfg.buffer.frameCount = count;

    setInBuffer(static_cast<int16_t *>(buffer));
    setOutBuffer(static_cast<int16_t *>(buffer));
...
//#mohongwu@wind-mobi.com begin Feature#110179
#ifdef WIND_DEF_AUDIOWIZARD
void AudioFlinger::EffectModule::process_l()
{
     if (mState == DESTROYED || mEffectInterface == NULL ||
            mConfig.inputCfg.buffer.raw == NULL ||
            mConfig.outputCfg.buffer.raw == NULL) {
...
//#mohongwu@wind-mobi.com begin Feature#110179
#ifdef WIND_DEF_AUDIOWIZARD
status_t AudioFlinger::EffectHandle::reset()
{
	if (mEffect == 0) {
		return DEAD_OBJECT;
	}

	return mEffect->command(EFFECT_CMD_RESET, 0, 0, 0, 0);
}
#endif
//#mohongwu@wind-mobi.com end Feature#110179
