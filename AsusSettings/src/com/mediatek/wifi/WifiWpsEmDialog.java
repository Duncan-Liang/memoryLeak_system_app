...
public class WifiWpsEmDialog extends AlertDialog
        ...
        public WifiWpsEmDialog(Context context,
            AccessPoint accessPoint, boolean isOnlyVisibilityWpsPin) {
            super(context, 0);//R.style.Theme_WifiDialog);
        ...
        protected void onCreate(Bundle savedInstanceState) {
            ...
            ((CheckBox) mView.findViewById(R.id.wifi_defalut_pins_togglebox)).
        ...
        public void onClick(View view) {
             if (view.getId() == R.id.wifi_defalut_pins_togglebox) {
                  Log.d("@M_" + TAG, "onClick wifi_defalut_pins_togglebox clicked "
                      + "hide PIN items");
                  ...
                  Log.d("@M_" + TAG, "onClick nfc_password_token_togglebox clicked "
                      + "disable PIN items");
                  Log.d("@M_" + TAG, "nfc_password_token_togglebox is checked : "
                      + ((CheckBox) view).isChecked());
                  ((EditText) mView.findViewById(R.id.wifi_pin_code)).
        ...
        public void onClick(DialogInterface dialogInterface, int button) {
            if (button == BUTTON_SUBMIT) {
                 Log.d("@M_" + TAG, "onClick, save configuration");
                  mWpsListener = new WifiManager.WpsCallback() {
                    public void onStarted(String pin) {
                    }
                    public void onSucceeded() {
                        if (!mIsOnlyVisibilityWpsPin
                            && mPinSecuritySpinner.getSelectedItemPosition() == 0) {
                            Toast.makeText(mContext, R.string.wifi_open_mode,
                                Toast.LENGTH_LONG).show();
                        }
                    }
                    public void onFailed(int reason) {
                        Log.d("@M_" + TAG, "onFailed, the reason is :" + reason);
                ...
                if (mIsOnlyVisibilityWpsPin) {
                    if (mAccessPoint == null || mAccessPoint.networkId != INVALID_NETWORK_ID) {
                        Log.d("@M_" + TAG, "startWpsExternalRegistrar, config = "
                            + config.toString());
                        mWifiManager.startWpsExternalRegistrar(config, mWpsListener);
                    } else {
                        Log.d("@M_" + TAG, "startWps, config = " + config.toString());
                        config.BSSID = mAccessPoint.bssid;
                        mWifiManager.startWps(config, mWpsListener);
                    }
                } else {
                    CheckBox nfcPasswordCheckBox = (CheckBox) mView.
                        findViewById(R.id.nfc_password_token_togglebox);
                    if (!nfcPasswordCheckBox.isChecked() && (mPinCode.getText() == null
                        || "".equals(mPinCode.getText()))) {
                        Toast.makeText(mContext, "Please enter PIN code",
                            Toast.LENGTH_SHORT).show();

                    } else {
                        if (((CheckBox) mView.findViewById(R.id.nfc_password_token_togglebox)).
                            isChecked()) {
                        ...
                        config.key = mKey.getText().toString();
                         ...
                         Log.d("@M_" + TAG, "startWpsRegistrar, config = " + config.toString());
