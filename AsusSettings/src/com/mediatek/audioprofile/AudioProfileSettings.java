...
// wangyan@wind-mobi.com add 2016/05/26 for Feature #110152 -s
import android.os.SystemProperties;
// wangyan@wind-mobi.com add 2016/05/26 for Feature #110152 -e
...
public class AudioProfileSettings extends SettingsPreferenceFragment implements
    ...
    private final AudioProfileListener mListener = new AudioProfileListener() {
        ...
        public void onProfileChanged(String profileKey) {
            ...
            Log.d("@M_" + XLOGTAG, TAG + "onPerfileChanged:key " + profileKey);
    ...
    private final BroadcastReceiver mBroadcastReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (Intent.ACTION_USER_SWITCHED.equals(action)) {
                Log.d("@M_" + XLOGTAG, "User switched");
                mIsSwitchUser = true;
            }
        }
    };
    ...
    public void onCreate(Bundle icicle) {
        Log.d("@M_" + XLOGTAG, "onCreate");
        super.onCreate(icicle);
        mContext = getActivity();

        mExt = UtilsExt.getAudioProfilePlugin(getActivity());
        ...
        if (WIND_DEF_ASUS_NOT_DISTURB) {
            if (findPreference(KEY_ZEN_MODE) != null)
                getPreferenceScreen().removePreference(findPreference(KEY_ZEN_MODE));
        }
        // wangyan@wind-mobi.com add 2016/05/26 for Feature #110152 -e
    ...
    private void updatePreferenceHierarchy() {
        ...
        if (profileKeys == null) {
            Log.d("@M_" + XLOGTAG, TAG + "profileKey size is 0");
            return;
        }
        Log.d("@M_" + XLOGTAG, TAG + "profileKey size" + profileKeys.size());
    ...
    private AudioProfilePreference addPreference(PreferenceScreen root,
        ...
        if (Scenario.CUSTOM.equals(scenario)) {
            ...
            Log.d("@M_" + XLOGTAG, TAG + "Add into profile list " + preference.getKey());

            if (preference.getKey().equals(mEditProfileKey) && mPref != null)
            {
                 Log.d("@M_" + XLOGTAG, TAG + "resume mPref: rename profile, key = "
                         + mEditProfileKey);
                 mPref = preference;
            }
            ...
            Log.d("@M_" + XLOGTAG, TAG + "Add into profile preference " + preference.getKey());

            String name = mProfileManager.getProfileName(key);

            if (name != null) {
                preference.setTitle(name, false);
                Log.d("@M_" + XLOGTAG, TAG + String.valueOf(preference.getTitle()));
            }
        }
        return preference;
    }
    ...
    private final View.OnClickListener mProfileSettingListener = new View.OnClickListener() {
        public void onClick(View v) {
            ...
            Log.d("@M_" + XLOGTAG, "on Click ImageView: " + key);
    ...
    private void updateActivePreference() {
        ...
        Log.d("@M_" + XLOGTAG, TAG + "key " + key);
        AudioProfilePreference activePreference =
                (AudioProfilePreference) findPreference(key == null ? mDefaultKey : key);
        if (activePreference != null) {
            activePreference.setChecked();
        }
    }
    ...
    /**
     * called when the fragment is invisible to the user unregister the profile
     * change listener.
     */
    public void onPause() {
        super.onPause();
    ...
    /**
     * For General profile, long click to enable it for customer profile,
     * long click to enable/rename/delete it.
     *
     * @param item
    ...
    public boolean onContextItemSelected(final MenuItem item) {
        ...
        if (pref instanceof AudioProfilePreference) {
            ...
            Log.d("@M_" + XLOGTAG, "onContextItemSelected  mPref = " + mPref.toString()
                    + " mEditProfileKey = " + mEditProfileKey);
    ...
    public void onClick(DialogInterface dialogInterface, int button) {
        Log.d("@M_" + XLOGTAG, "onClick");
        Log.d("@M_" + XLOGTAG, "" + button);
        if (button != DialogInterface.BUTTON_POSITIVE) {
            Log.d("@M_" + XLOGTAG, "return");
            return;
        }
        ...
        case DIALOG_NAME:
            ...
            if (title.length() == 0) {
                ...
                if (mMenuId == MENUID_ADD) {
                    ...
                    Log.d("@M_" + XLOGTAG, TAG + "add profile Key" + profileKey);
                    ...
                    Log.d("@M_" + XLOGTAG, "onClick  mPref.setTitle = " + title);
        ...
        case DIALOG_RESET:
            ...
            break;
        default:
            Log.d("@M_" + XLOGTAG, TAG + "unrecongnized dialog id is"
                    + mCurrentDialogId);
            break;
        }
    }
    ...
    private boolean dismissFrag() {
        Log.d("@M_" + XLOGTAG, "Dismiss fragment.");
        if (mIsSwitchUser == true) {
            mIsSwitchUser = false;
            Log.d("@M_" + XLOGTAG, "AudioProfileSettings will be finished.");
            finish();
            return true;
        }
        return false;
