...
//add by sunxiaolong@wind-mobi.com for asus storage showing begin
import android.net.TrafficStats;
import android.os.SystemProperties;
//add by sunxiaolong@wind-mobi.com for asus storage showing end
...
public class StorageSettings extends SettingsPreferenceFragment implements Indexable {
    ...
    //add by sunxiaolong@wind-mobi.com for asus storage showing begin
    private static final boolean ASUS_STORAGE_SHOW = SystemProperties.getBoolean("ro.wind.def.asus_storage", false);
    //add by sunxiaolong@wind-mobi.com for asus storage showing end
    ...
    private void refresh() {
        ...
        for (VolumeInfo vol : volumes) {
            if (vol.getType() == VolumeInfo.TYPE_PRIVATE) {
                ...
                //add by sunxiaolong@wind-mobi.com for asus storage showing begin
                if (ASUS_STORAGE_SHOW) {
                    mInternalCategory.addPreference(
                            new AsusStorageSystemVolumePrefernce(context, vol, Color.parseColor("#990099")));
                }
                //add by sunxiaolong@wind-mobi.com for asus storage showing end
        ...
        //modify by sunxiaolong@wind-mobi.com for asus storage show begin
        if (ASUS_STORAGE_SHOW) {
            final long totalFlashBytes = privateTotalBytes > 16 * TrafficStats.GB_IN_BYTES ? 32 * TrafficStats.GB_IN_BYTES
                    : 16 * TrafficStats.GB_IN_BYTES;
            long mSystemRetainBytes = totalFlashBytes - privateTotalBytes;
            mInternalSummary.setTitle(Formatter.formatFileSize(context, totalFlashBytes
                    - mSystemRetainBytes - privateUsedBytes) + " " + getString(R.string.memory_available));
            mInternalSummary.setSummary(getString(R.string.storage_volume_summary,
                    Formatter.formatFileSize(context, mSystemRetainBytes + privateUsedBytes),
                    Formatter.formatFileSize(context, totalFlashBytes)));
        } else {
        ...
        //modify by sunxiaolong@wind-mobi.com for asus storage show end
        ...
        //modify by sunxiaolong@wind-mobi.com for asus storage showing begin
        //if (mInternalCategory.getPreferenceCount() == 2
        //        && mExternalCategory.getPreferenceCount() == 0 && !sHasOpened) {
        if (mInternalCategory.getPreferenceCount() == 2
                && mExternalCategory.getPreferenceCount() == 0 && !sHasOpened && !ASUS_STORAGE_SHOW) {
        //modify by sunxiaolong@wind-mobi.com for asus storage showing end
    ...
    public boolean onPreferenceTreeClick(PreferenceScreen preferenceScreen, Preference pref) {
        ...
        if (pref instanceof StorageVolumePreference) {
            ...
            return true;
        //add by sunxiaolong@wind-mobi.com for asus's storage show begin
        } else if(key.equals("asus_system_volume")){
            return true;
        }else {
        //add by sunxiaolong@wind-mobi.com for asus's storage show end
            ///M:
