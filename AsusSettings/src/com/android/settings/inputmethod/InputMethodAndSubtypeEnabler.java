...
//ADD by sunxiaolong@wind-mobi.com for asus patch begin
import android.widget.Toast;
//ADD by sunxiaolong@wind-mobi.com for asus patch end
...
public class InputMethodAndSubtypeEnabler extends SettingsPreferenceFragment
    ...
    //ADD by sunxiaolong@wind-mobi.com for asus patch begin
    //BEGIN: adrian_sung@asus.com
    private static final int MAX_ASUS_SUBTYPE_NUM = android.os.SystemProperties.getInt("ro.config.xt9ime.max_subtype", 8);
    private static final String ASUS_IME_IMI_ID = "com.asus.ime/.IME";
    private int mAsusSubtypeCount = 0;
    ...
    public void onResume() {
        ...
        //ADD by sunxiaolong@wind-mobi.com for asus patch begin
        //BEGIN: adrian_sung@asus.com : limit the number of ASUS subtype
        for (int i = 0; i < mInputMethodInfoList.size(); i++) {
            InputMethodInfo imi = mInputMethodInfoList.get(i);
            if (isCurrentAsusImi(imi)) {
                List<InputMethodSubtype> enabledSubtypes =
                        mImm.getEnabledInputMethodSubtypeList(imi, true);
                mAsusSubtypeCount = enabledSubtypes.size();
            }
        }
        //END: adrian_sung@asus.com
        //ADD by sunxiaolong@wind-mobi.com for asus patch end
    ...
    public boolean onPreferenceTreeClick(PreferenceScreen preferenceScreen,
        ...
        final TwoStatePreference chkPref = (TwoStatePreference) preference;
        if (chkPref.isChecked()) {
            final int N = mInputMethodInfoList.size();
            for (int i = 0; i < N; i++) {
                InputMethodInfo imi = mInputMethodInfoList.get(i);
                if (isCurrentAsusImi(imi) && preference != mAutoSelectionPrefsMap.get(imi.getId())) {
                    if (mAsusSubtypeCount >= MAX_ASUS_SUBTYPE_NUM) {
                        chkPref.setChecked(false);
                        Toast.makeText(getActivity(), R.string.asus_keyboard_subtype_limit, Toast.LENGTH_SHORT).show();
                        return true;
                    } else {
                        if (!mAutoSelectionPrefsMap.get(imi.getId()).isChecked()) {
                            mAsusSubtypeCount++;
                        }
                    }
                }
    ...
    private void addInputMethodSubtypePreferences(final InputMethodInfo imi,
        ...
        //ADD by sunxiaolong@wind-mobi.com for asus patch begin
        //BEGIN: adrian_sung@asus.com, group emoji and emoticons subtype on the top of list
        if (isCurrentAsusImi(imi)) {
            for (int index = 0; index < prefCount; ++index) {
                final Preference pref = subtypePreferences.get(index);
                if (subtypePreferences.get(index).getTitle().equals("Emoji") ||
                        subtypePreferences.get(index).getTitle().equals("Emoticons")) {
                    activeInputMethodsCategory.addPreference(pref);
                    pref.setOnPreferenceChangeListener(this);
                }
            }
        }
        //END: adrian_sung@asus.com, group emoji and emoticons subtype on the top of list
        //ADD by sunxiaolong@wind-mobi.com for asus patch end
        for (int index = 0; index < prefCount; ++index) {
            final Preference pref = subtypePreferences.get(index);
            activeInputMethodsCategory.addPreference(pref);
    ...
    private void updateImplicitlyEnabledSubtypesOf(final InputMethodInfo imi, final boolean check) {
        ...
        //ADD by sunxiaolong@wind-mobi.com for asus patch begin
        if (isCurrentAsusImi(imi)) {
            mAsusSubtypeCount = implicitlyEnabledSubtypes.size();
        }
        //ADD by sunxiaolong@wind-mobi.com for asus patch end
    }
    ...
    //ADD by sunxiaolong@wind-mobi.com for asus patch begin
    //BEGIN: adrian_sung@asus.com
    private boolean isCurrentAsusImi (InputMethodInfo imi) {
        if (imi.getPackageName() != null) {
            return imi.getPackageName().equals("com.asus.ime") &&  mInputMethodId.equals(imi.getId());
        } else {
            // BEGIN: adrian_sung@asus.com, fix TT-532393 NPE crash
            return false;
            // END:adrian_sung@asus.com
        }
    }
