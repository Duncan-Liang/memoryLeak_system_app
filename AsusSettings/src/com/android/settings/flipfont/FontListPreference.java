...
public class FontListPreference extends ListPreference {
    ...
    */
     ...
     * Prepares the font list dialog using specific adapter 
     */
    protected void onPrepareDialogBuilder(AlertDialog.Builder builder) {
        super.onPrepareDialogBuilder(builder);

        if (mListDialogIsActive) {
            if (DEBUG)
                Log.v("FlipFont", "onPrepareDialogBuilder() - dismissList() ");
            dismissList();
        }
        mListDialogIsActive = true;

        if (DEBUG)
            Log.v("FlipFont", "onPrepareDialogBuilder  (AlertDialog.Builder builder) ");
        ...
        String sCurrentSetFontName = Typeface.getFontNameFlipFont(context, 1);
        if (sCurrentSetFontName != null) {
            if (DEBUG)
                Log.v("FlipFont", "onPrepareDialogBuilder  sCurrentSetFontName=" + sCurrentSetFontName);
            for (int index = 0; index < mFontListAdapter.getCount(); index++) {
                String fontName = mFontListAdapter.getFontName(index);
                if (DEBUG)
                    Log.v("FlipFont", "onPrepareDialogBuilder  mFontListAdapter.getFontName(" + index + ")=" + fontName);
                if (sCurrentSetFontName.equals(fontName)) {
                    mPreviousFont = index;
                    break;
                }
            }
        ...
        if (DEBUG)
            Log.v("FlipFont", "onPrepareDialogBuilder  mPreviousFont=" + mPreviousFont);
        if (mPreviousFont < 0) {
            // if previous font not present select none
            mPreviousFont = -1;
        }
        builder.setSingleChoiceItems(mFontListAdapter, mPreviousFont, this);

        if (NO_CANCEL) {
            //Modify by Carol_Hu. Remove 'Get fonts online' button. Replaced by 'Cancel'
            //builder.setNegativeButton(R.string.monotype_dialog_button, new DialogInterface.OnClickListener() {
            builder.setNegativeButton(R.string.cancel, new DialogInterface.OnClickListener() {
                public void onClick(DialogInterface dialog, int whichButton) {
                    // If Font select dialog is showing, cancel it 
                    if (SetFontToAlert != null)
                        if (SetFontToAlert.isShowing())
                            SetFontToAlert.cancel();
                    mFontListAdapter = null;
                    //mBuyButtonClicked = true; //Modify by Carol_Hu. Remove 'Get fonts online' button. Replaced by 'Cancel'
                    restartLater(); // The user is canceling the font list dialog
                }
            });
        } else {
            builder.setPositiveButton(R.string.monotype_dialog_button, new DialogInterface.OnClickListener() {
                public void onClick(DialogInterface dialog, int whichButton) {
                    // If Font select dialog is showing, cancel it
                    if (SetFontToAlert != null)
                        if (SetFontToAlert.isShowing())
...
*/
     ...
     * Shows a simple question dialog to ask user whether (s)he wants to flip the font.   
     */
    public void onClick(DialogInterface dialog, int which) {
        if (DEBUG)
            Log.v("FlipFont", "onClick(DialogInterface dialog, int which) - which = " + which);
        super.onClick(dialog, which);

        // User has selected the same font already used, no action
        if (which == mPreviousFont) {
            if (DEBUG)
                Log.v("FlipFont", "onClick(DialogInterface dialog, int which) - exit ");
            return;
        }
        if (DEBUG)
            Log.v("FlipFont", "onClick(DialogInterface dialog, int which) after (which == mPreviousFont) ");

        // User has pressed the buy button
        if (which == -2) {
            if (DEBUG)
                Log.v("FlipFont", "onClick(DialogInterface dialog, int which) after (which == -2) ");
            if (SetFontToAlert != null)
                if (SetFontToAlert.isShowing())
                    SetFontToAlert.cancel();
            dismissApp();
     ...
     * Opens the android market, monotype_android_market_uri contains the actual uri
     */
    public void onDialogClosed(boolean positiveResult) {
        super.onDialogClosed(positiveResult);

        if (mBuyButtonClicked) {
            String uri = (String) context.getResources().getText(R.string.monotype_android_market_uri);
            try {
                Intent intent = new Intent().setAction(Intent.ACTION_VIEW).setData(Uri.parse(uri));
                context.startActivity(intent);
            } catch (android.content.ActivityNotFoundException ex) {
                Log.v("FlipFont", "onDialogClosed():" + ex);
            }
            mBuyButtonClicked = false;
        }
    }
