...
import com.android.settings.zenmotion.AppLaunchListPreference;
import com.android.settings.zenmotion.Utils;
...
public class TouchGestureSwitchEnabler extends AbstractEnabler implements
    ...
    private static final String PERSIST_ASUS_SWIPE = "persist.asus.swipeup";
    ...
    private void handleStateChanged() {
        Boolean enable = getSystemPropGestureIsChecked() || getDoubleTapOnIsChecked()
                || getDoubleTapOffIsChecked();
        setSwitchChecked(enable);
        Utils.DriverGesturesMainSwitch(enable);
    }
    ...
    private void writeDBSwitchOn() {
        ...
        int doubleTapOffValue = mZenMotionSharesPreferences.getInt(DOUBLE_TAP_OFF_DATA,
                DISABLE_DOUBLE_TAP_MODE);
        int swipeUpValue = mZenMotionSharesPreferences.getInt(SWIPE_UP_DATA,
                DISABLE_DOUBLE_TAP_MODE);
        writeDoubleTapOnDB(doubleTapOnValue);
        writeDoubleTapOffDB(doubleTapOffValue);
        writeSwipeUpDB(swipeUpValue);
        ...
        Utils.DriverGesturesItemSwitch(AppLaunchListPreference.KEY_DOUBLE_ON, intToBoolean(doubleTapOnValue));
        Utils.DriverGesturesItemSwitch(AppLaunchListPreference.KEY_SWIPE_UP, intToBoolean(swipeUpValue));
        Utils.DriverGesturesMainSwitch(true);
    }
    ...
    private void writeDBSwitchOff() {
        ...
        writeSwipeUpDB(DISABLE_DOUBLE_TAP_MODE);
        ...
        Utils.DriverGesturesMainSwitch(false);
    }
    ...
    private Boolean getSwipeUpIsChecked() {
        String stringValue = SystemProperties.get(PERSIST_ASUS_SWIPE,
                String.valueOf(DISABLE_DOUBLE_TAP_MODE));
        return stringValue.equals(String.valueOf(ENABLE_DOUBLE_TAP_MODE));
    }
    ...
    // wangyan@wind-mobi.com add 2016/06/24 for Feature#110195 start
