...
import android.os.SystemProperties;//A: huangyouzhong@wind-mobi.com 20160525 : Asus LockScreen Settings
...
import android.hardware.fingerprint.FingerprintManager; //asus add for fongerprint 20160701
...
public final class Utils {
	...
	//asus add for fingerprint 20160701 end
    /**
    ...
    public static boolean copyMeProfilePhoto(Context context, UserInfo user) {
        ...
        //M: Mark for CR ALPS00438553 fix.
        //int userId = user != null ? user.id : UserHandle.myUserId();
        //UserManager um = (UserManager) context.getSystemService(Context.USER_SERVICE);
    ...
    public static void inflateOriginalLayout(Activity activity,
            LayoutInflater inflater, int layoutId) {
        inflater.inflate(layoutId,
                (LinearLayout) activity.findViewById(R.layout.colorful_bar));
    }
    ...
    public static void updateColorfulTextView(Activity activity,
            int backgroundRes, int colorfulViewId) {
        TextView textViewColorful = (TextView) activity
                .findViewById(colorfulViewId);
        if (textViewColorful == null) {
            android.util.Log.e("Utils", "can not get colorful view");
            return;
        }

        if (supportWindowTranslucentStatus(activity)) {
            int statusH = getStatusBarHeight(activity);
            int actionbarH = getActionBarHeight(activity);
            textViewColorful.setHeight(statusH + actionbarH);
            textViewColorful.setBackgroundResource(backgroundRes);
        } else {
    ...
    public static boolean isPrivateMode(UserManager um) {
    ...
    public static int getPrivateUserId(UserManager um) {
        //final UserManager um = (UserManager) getActivity().getSystemService(Context.USER_SERVICE);
        final List<UserInfo> users = um.getUsers(true);
        final int totalUserCount = users.size();
        int UserId = -1;
    ...
    public static boolean isUserEnable(UserManager um ,int UserId) {
        //final UserManager um = (UserManager) getActivity().getSystemService(Context.USER_SERVICE);
        final List<UserInfo> users = um.getUsers(true);
        final int totalUserCount = users.size();
        for (int i = 0; i < totalUserCount; i++) {
   ...
   private static boolean isProfileOf(UserManager um, UserHandle otherUser) {
    ...
    public static boolean isSystemPackage(PackageManager pm, PackageInfo pkg) {
        ...
        return sSystemSignature[0] != null && sSystemSignature[0].equals(getFirstSignature(pkg)) && !pkg.packageName.equals("com.cmcm.skey");
    }
  ...
  //+++ Jason_Uang
    ...
    public static boolean isOnStationMode(Context context) {
        PackageManager pm = context.getPackageManager();
        final boolean hasPadfoneStationFeatureEC = false; //pm.hasSystemFeature(PackageManager.FEATURE_ASUS_FIRMWARE_PADFONE_STATION_PAD_EC);
        final boolean hasPadfoneStationFeatureTouch = false;//pm.hasSystemFeature(PackageManager.FEATURE_ASUS_FIRMWARE_PADFONE_STATION_TOUCH);
    ...
    public static boolean isCNSKU(){
        ...
        return SKU.toLowerCase().equals("cn") ||
                SKU.toLowerCase().equals("cta") ||
                SKU.toLowerCase().equals("cucc") ||
                SKU.toLowerCase().equals("cmcc") ||
                SKU.toLowerCase().equals("iqy");
    }
    ...
    public static boolean hasGooglePlay(Context context){
        PackageManager pm = context.getPackageManager();
        List<ResolveInfo> launchableApps = pm.queryIntentActivities(new Intent(
                Intent.ACTION_MAIN).addCategory(Intent.CATEGORY_LAUNCHER), 0);
        for (ResolveInfo app:launchableApps){
            if(app.activityInfo.packageName.equals(GOOGLE_PLAY_STORE_PACKAGE_NAME)) return true;
        }
        return false;
    }
    ...
    // +++ Kevin_Chiou Check Theme app exist or not
    public static boolean isThemeAppExist(Context context) {
        PackageManager pm = context.getPackageManager();
        try {
            pm.getPackageInfo(THEME_APP_PACKAGE_NAME, PackageManager.GET_ACTIVITIES);
            return true;
        } catch (NameNotFoundException e) {
            return false;
        }
    ...
    public static boolean isAsusApkWW() {
        return SystemProperties.get("ro.wind.def.adapt_asus_apk_ww").equals("1");
    }
    public static boolean isAsusApkCN() {
        return SystemProperties.get("ro.wind.def.adapt_asus_apk_cn").equals("1");
    }
